...
export function ProfilePanel() {
  ...
  const avatarUploadProgress = useUploadProgress(
    avatarBusy && Boolean(avatarFile),
    avatarFile ? `${avatarFile.name}-${avatarFile.size}-${avatarFile.lastModified}` : "none",
  );
        ...
        <CardContent>
          <div className="mb-6 rounded-xl border border-border/70 bg-muted/20 p-4">
            <div className="flex flex-wrap items-center gap-4">
              <UserAvatar src={avatarPreviewUrl ?? profile?.avatarUrl ?? null} alt="ØªØµÙˆÛŒØ± Ù¾Ø±ÙˆÙØ§ÛŒÙ„" className="h-20 w-20" />
              <div className="flex-1 space-y-2">
                ...
                />
                <UploadLoading active={avatarBusy && Boolean(avatarFile)} progress={avatarUploadProgress} file={avatarFile} />
          ...
          <form className="grid gap-5 rounded-xl border border-border/70 bg-muted/20 p-4 md:grid-cols-2" onSubmit={saveProfile}>
